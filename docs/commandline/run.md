...
the host machine. The [Docker User Guide](../../userguide/networking/dockerlinks.md)
explains in detail how to manipulate ports in Docker.

    $ docker run --expose 80 ubuntu bash

This exposes port `80` of the container fwithout
publishing the port to the host system's interfaces.
...
containers can communicate easily needinly only another container's IP address
