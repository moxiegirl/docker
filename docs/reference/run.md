...
The UUID identifiers come from the Docker daemon, and if you do not assign a
name to the container with `--name` then the daemon will also generate a random
string name too. The `name` can become a handy way to add meaning to a container
since you can use this `name` when referencing the container within a Docker
network. This works for both background and foreground Docker containers.
...
You can create a network using a Docker network driver or external network
driver plugin. You can connect multiple containers to the same network. Once
connected, the containers can communicate easily using only another container's
IP address or name.  
...
Linking is a legacy feature, you should prefer Docker networks instead. For more
information on Docker networks, see the [*Docker network
